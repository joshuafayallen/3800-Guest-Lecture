...
pacman::p_load("ggplot2", "AllenMisc", "magrittr","dplyr", "flair", "MetBrewer", "colorspace", "gapminder", "sf",  install = FALSE)
...
## sets the working directory on mac
setwd("your/working/directory/here/")
# sets the working directory on windows 
setwd("your\working\directory\here") 
```
...
When we organize our files into HW 1 or manuscript whatever name what we are doing is creating a new neighborhood on our computer. R will default to places it knows. Most commonly where it lives. In order to do something as simple as loading our dataset in R needs directions to this neighborhoo
...
- You have the option of pointing and clicking via import dataset
  - tedious on its best day

- Do it via code!
  - It makes you way lazier

- These are  common errors you will get  
...
- In this case the file was hiding in the data folder
...
- Now that we have an object we need to know how to navigate to stuff in it
  - the easiest way is using `$`
  - This lets us use the names of things in our object
...
::: incremental
- To navigate our object we do `objectname[things, things]`
- `[]` has two slots
    - `[row,column]`
:::
:::{.fragment}
...
## Very Basic Analysis {.scrollable}
- Our $H_0$ is that the body mass of  Adelie Penguins are not different then other penguins
- and our $H_a$ is that the body mass of Adelie Penguins are different than other penguins 
...
- Next we can use a simple t-test
...
```{r}
t.test(body_mass_g ~ gentoo, data = penguins_t_test)
```
...
:::{.notes}
Position scales are used to control the locations of visual entities in a plot, and how those locations are mapped to data values. Every plot has two position scales, corresponding to the x and y aesthetics. In most cases this is clear in the plot specification, because the user explicitly specifies the variables mapped to x and y explicitly. 

Colors are also another form of a scale. To keep it simple basically what is going on is that R is mapping each point point to a specific color. For both kinds of color scales this is done via fitting to the point through a colorspace 
...
::: {layout-ncol="2"}
```{r theme-minimal-theme-out, echo = TRUE, out.width = "80%"}
#| code-fold: true
ggplot(gapminder, 
       aes(x = gdpPercap,
        y = lifeExp, 
...
<figure>
  <img src="https://raw.githubusercontent.com/andrewheiss/2021-seacen/main/02_data-visualization/slides/img/bbc-cookbook.png" alt="ggthemes" title="ggthemes" width="100%">
</figure>
...
labs(title = "Health and wealth grow together",
     ...
     size = "Population",
  caption = "Source: The Gapminder Project") +
    theme_allen_bw(has_subtitle = TRUE)
```
...
ggplot(gapminder, 
  ...
  labs(title = "Health and wealth grow together",
      ...
      size = "Population",
      caption = "Source: The Gapminder Project") +
       theme_allen_bw(base_size = 45, has_subtitle = TRUE)
```
...
ggsave("name-of-your-file.png",your_plot_here)
```
